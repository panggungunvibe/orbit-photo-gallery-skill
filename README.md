...
智能体会读取这个相册，筛选照片、去重分类、写日记，确认至少能自然分出五类后，直接编写并运行网站，给你一个能打开的预览。你不需要先手动下载照片、填写选片清单，也不需要会写代码。
...
- 手机保留圆环、长卷、收藏和大图，隐藏手势开关；支持系统保存菜单或长按图片保存。

默认排除横图和方图；近似连拍择优保留一张，模糊与失败表情排除，每张照片只归一类。脚本检查至少五类、方向和精确重复，最佳表情、近似去重与分类语义由视觉审阅判断，不能靠脚本计数凑类。
...
智能体会完成下载、看图、选片、分类、文案、coding、测试和启动预览。若筛选后不足五类，会说明现有类目并请你补充照片；不硬拆类别、不重复用图凑数，也不先生成效果不完整的网站。相册重名、范围不明或授权缺失时，也会补问必要信息。不会修改网盘原片，也不会自动公开你的照片。
...
让模型查看联系表和候选原片，先筛选为至少五个自然、非空的分类，按照 [清单格式](references/manifest.md) 编写私人 `selection.json`，再运行：
